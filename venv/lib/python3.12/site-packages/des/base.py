...
def encode(block, key, encryption, firstBlock):
    for k in key:
        block = encode_block(block, k, encryption, firstBlock)
...
def handle(message, key, initial, padding, encryption):
    ...
    if initial is None:
        # ECB THIS <-- electronic code block mode
...
def ecb(blocks, key, encryption):
    ...
    for i, block in enumerate(blocks):
        if i != 0:
            firstBlock = False # used as a flag for printing only the first block
        initial = encode(block, key, encryption, firstBlock)
        yield initial
...
def cbc(blocks, key, initial, encryption):
    ...
    if encryption:
        for block in blocks:
            initial = encode(block ^ initial, key, encryption, firstBlock)
            yield initial
    else:
        for block in blocks:
            initial, block = block, initial ^ encode(block, key, encryption, False) # I do not need it to print stuff when decrypting
            yield block
